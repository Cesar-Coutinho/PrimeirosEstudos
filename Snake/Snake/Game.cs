    ...
    class Game
    {
        ...
        public void Tick()
        {
            ...
            for (int i = 0; i < Snake.Length; i++)
            {
                if(i == 0)
                {
                    bitmapGraph.FillEllipse(new SolidBrush(ColorTranslator.FromHtml("#f00000")), (Snake.Location[i].X * 15), (Snake.Location[i].Y * 15), 15, 15);
                }
                else
                {
                    bitmapGraph.FillEllipse(new SolidBrush(ColorTranslator.FromHtml("#ff00f0")), (Snake.Location[i].X * 15), (Snake.Location[i].Y * 15), 15, 15);
                }
        ...
        public void GameOver()
        {
            ...
            bitmapGraph.Dispose();
            screenGraph.Dispose();
            LbPonto.Text = "PONTOS:0";
            MessageBox.Show("GAME OVER","LOSEER",MessageBoxButtons.OK,MessageBoxIcon.Exclamation);
            Pontos = 0;
        }
    }
}
